(function () {
    angular.module('util-Filtros', [])
        ...
        }).filter('trans', ['$rootScope',function ($rootScope) {
            var tables = {'es': {'TITULO': 'Controla tu Cash Flow'},'en': {'TITULO': 'Take control of your Cash Flow'}};
            $rootScope.idioma = 'en';
            return function (input) {
                if (input) {
                    return tables[$rootScope.idioma][input];
                };
            }
        }]);
}());